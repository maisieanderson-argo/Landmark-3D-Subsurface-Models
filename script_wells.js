...
function parseWellCSV(text) {
    const lines = text.split('\n');
    const wells = {}; // "WellName" -> [{x,y,z,md}, ...]
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const md = parseFloat(parts[4]);
        
        if (isNaN(x) || isNaN(y) || isNaN(z)) continue;
        
        if (!wells[name]) wells[name] = [];
        wells[name].push({ x, y, z, md });
    }
    return wells;
}

async function loadWells(url) {
    updateLoading("Loading Wells...");
    try {
        const response = await fetch(url);
        if (!response.ok) throw new Error(response.statusText);
        const text = await response.text();
        return parseWellCSV(text);
    } catch (e) {
...
async function initVolveData() {
    ...
    // Load and Render Wells
    const wellsData = await loadWells("wells.csv");
    Object.keys(wellsData).forEach(wellName => {
        const points = wellsData[wellName];
        if (points.length < 2) return;
        
        // Convert to Three.js coords (same transform as terrain)
        const curvePoints = points.map(p => new THREE.Vector3(
            p.x - centerX, 
            -p.z, 
            -(p.y - centerY)
        ));
        ...
        // radius = 5m equivalent (since scale is 1 unit = 1 meter approx)
        const tubeGeometry = new THREE.TubeGeometry(curve, Math.min(200, points.length * 5), 10, 8, false);
        const tubeMaterial = new THREE.MeshStandardMaterial({ 
            color: 0xFFD700, // Gold/Yellow
            roughness: 0.3,
            metalness: 0.8
        }); 
        const wellMesh = new THREE.Mesh(tubeGeometry, tubeMaterial);
        
        wellMesh.userData.isWell = true;
        wellMesh.userData.wellName = wellName;
        modelGroup.add(wellMesh);
    });
...
function applyState(state) {
    ...
    // Wells
    modelGroup.children.forEach(c => {
        if (c.userData.isWell) {
            c.visible = params.showWells;
        }
    });
...
function updateColoring() {
    // 1. Update Well Colors (Use last color of colormap)
    const stops = ColormapRegistry[params.selectedColormap] || ColormapRegistry['Viridis'];
    const lastColor = stops[stops.length - 1].c;

    modelGroup.children.forEach(mesh => {
        if (mesh.userData.isWell) {
            mesh.material.color.set(lastColor);
    ...
    if (params.colorByDepth) {
        ...
        modelGroup.children.forEach(mesh => {
            if (mesh.userData.isContour || mesh.userData.isWell) return; // Skip contours AND wells
        ...
        // Apply vertex colors
        modelGroup.children.forEach(mesh => {
            if (mesh.userData.isContour || mesh.userData.isWell) return;
...
vizFolder.add(params, 'showWells').name('Show Wells').onChange((v) => {
    modelGroup.children.forEach(c => {
        if (c.userData.isWell) {
            c.visible = v;
        }
